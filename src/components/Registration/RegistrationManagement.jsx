...
import './RegistrationManagement.scss';
...
const RegistrationManagement = () => {
  ...
  useEffect(() => {
    const fetchRegistrations = async () => {
      try {
        const data = await registrationServiceAPI.getRegistrations();
  ...
  const handleFirstRoundApproval = async () => {
    if (selectedRegistration) {
      ...
      try {
        await registrationServiceAPI.updatePassedFirstRound(selectedRegistration._id, formDataToSend);
  ...
  const handleSecondRoundApproval = async (registrationId) => {
    try {
      await registrationServiceAPI.updatePassedSecondRound(registrationId);
      setRegistrations(prevRegistrations =>
        prevRegistrations.map(registration =>
  ...
  const confirmDelete = async () => {
    try {
      await registrationServiceAPI.deleteRegistration(registrationToDelete);
      setRegistrations(prevRegistrations =>
